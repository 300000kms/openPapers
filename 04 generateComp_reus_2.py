...
project = 'Recompte Tarragona 2017'
...
def mkChapter(b):
    ...
    #print '"name" = \''+b.attribute('name').replace("'", "''")+'\''
    #zz = Lz.getFeatures(QgsFeatureRequest(QgsExpression('"name" = \'Via Augusta_Musics_Valle de l\'\'Arrabassada_Ermita de la Salut_1\'')))#cuidado si el valor es integer o string

    zz = [x for x in zz]

    # escogemos el distrito correspondiente a la zona
    filter('districtes', '"name" = \''+str(zz[0].attribute('d_name'))+'\'' )
    print '"name" = \''+str(zz[0].attribute('d_name'))+'\'' 
...
def mkDiv(b,  z):
    ...
    ##barrio y distrito
    distrito = composition.getComposerItemById('distrito')
    distrito.setText(zz[0].attribute('name')+' | '+str(zz[0].attribute('d_name')))
    barrio = composition.getComposerItemById('barrio')
    barrio.setText(b.attribute('name'))
    ...
    image_name = b.attribute('name')+'_'+str(b.id())
    #composition.exportAsPDF(image_name+'.pdf')
    #image = composition.printPageAsRaster(0)
    #image.save(image_name+'.png','png')
...
def mkForm(b):
    ...
    ##guarda
    composition.refreshItems()
    image_name = b.attribute('name')+'_'+str(b.id())+'_form'
    #composition.exportAsPDF(image_name+'.pdf')
    #image = composition.printPageAsRaster(0)
    #image.save(image_name+'.png','png')
...
for b in Lb.getFeatures():
    print '>>>> chapter: ',
    print b.attributes()
    if x>=0 and x < 200: #para provocar una salida rapida , dejar 2000
        mkChapter(b)
