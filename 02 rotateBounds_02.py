...
def convexHull():
    ...
    c = conn.cursor()
    c.execute('select name , asgeojson(st_convexhull(geometry)), id from  elements')
    result =c.fetchall()
    return result
...
def createSqlite():
    ...
    try:
        ...
        print e
    c.execute('CREATE TABLE if not exists elements (id, name, ang)')
    conn.commit()
...
def toSqlite(idd, name, ang, bbx):
    conn = db.connect(r_db_bounds)
    c = conn.cursor()
    c.execute('''insert into elements ( id, name, ang, geometry ) values (?, ?, ?, GeomFromEWKT( ? ) )''', (idd, name, ang, "SRID=25831;"+str(bbx) ) )
    conn.commit()
    conn.close()
    return
