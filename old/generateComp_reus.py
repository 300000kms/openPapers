...
layers={
   ...
   'districtes' : ('bases/zones.sqlite', 'zones', 'geometry')
   }
...
def mkChapter(b):
    #print b.attributes()
    filter('zones', '"name" = "'+str(b.attributes()[0])+'"' )#cuidado si el valor es integer o string
    filter('zones2', '"name" = "'+str(b.attributes()[0])+'"' )#cuidado si el valor es integer o string
    ...
    print ('"name" = \''+str(b.attributes()[0])+'\'')
    zz = Lz.getFeatures(QgsFeatureRequest(QgsExpression('"name" = \''+str(b.attributes()[0])+'\'')))#cuidado si el valor es integer o string
    zz = [x for x in zz]
    print zz
    filter('districtes', '"c_distri" = '+str(zz[0].attributes()[0])+'' )
    ...
    distrito.setText(str(zz[0].attributes()[3])+' | '+str(zz[0].attributes()[2]))
    barrio = composition.getComposerItemById('barrio')
    barrio.setText(str(zz[0].attributes()[1]))#+' | '+str(zz[0].attributes()[4])) #ojo!
...
def mkDiv(b,  z):
    ...
    ##barrio y distrito
    distrito = composition.getComposerItemById('distrito')
    distrito.setText(str(zz[0].attributes()[1])+' | '+str(zz[0].attributes()[4]))
    barrio = composition.getComposerItemById('barrio')
    barrio.setText(str(b.attributes()[2])+' | '+str(b.attributes()[3]))
...
for b in Lb.getFeatures():
    print b
    if x>=0 and x < 2000: #para provocar una salida rapida , dejar 2000
